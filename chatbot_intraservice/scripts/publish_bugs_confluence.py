# -*- coding: utf-8 -*-
"""Публикация журнала «баги и исправления» в WEBKB.

  python scripts/publish_bugs_confluence.py

Родитель: страница проекта (124640056).
Источник: docs/bugs_and_fixes.md
"""
...
SPACE = "WEBKB"
TITLE = "Чатбот IntraService: баги и исправления"
SOURCE_MD = ROOT / "docs" / "bugs_and_fixes.md"
PROJECT_META = ROOT / "docs" / "confluence" / "intraservice_project_page_id.json"
OUT_META = ROOT / "docs" / "confluence" / "bugs_fixes_page_id.json"

BANNER = """
<ac:structured-macro ac:name="info" ac:schema-version="1"><ac:rich-text-body>
<p><strong>Журнал инцидентов</strong> чатбота 1 линии HelpDesk: ложные срабатывания,
регрессии и принятые исправления. Для операторов и разработчиков.</p>
<p>Исходник: <code>chatbot_intraservice/docs/bugs_and_fixes.md</code> ·
обновление: <code>python scripts/publish_bugs_confluence.py</code>.</p>
<p>См. также:
<ac:link><ri:page ri:space-key="WEBKB" ri:content-title="Чатбот IntraService: дополнительные настройки и идеи развития" /></ac:link>,
...
def main() -> int:
    ...
    if existing:
        ...
        result = cf.update_page(
            ...
            title=TITLE,
            storage=body,
            message="Sync bugs_and_fixes.md",
        )
    else:
