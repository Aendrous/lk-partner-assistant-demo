# -*- coding: utf-8 -*-
"""Публикация «Переход на n8n» в WEBKB.

  python scripts/publish_n8n_migration_confluence.py

Родитель: страница проекта (124640056 / ONttBw).
Источник: docs/интеграции/n8n_migration.md
"""
...
SPACE = "WEBKB"
TITLE = "Чатбот IntraService: переход на n8n (оркестрация)"
SOURCE_MD = ROOT / "docs" / "интеграции" / "n8n_migration.md"
PROJECT_META = ROOT / "docs" / "confluence" / "intraservice_project_page_id.json"
OUT_META = ROOT / "docs" / "confluence" / "n8n_migration_page_id.json"

BANNER = """
<ac:structured-macro ac:name="info" ac:schema-version="1"><ac:rich-text-body>
<p><strong>n8n</strong> заменяет Windows Task Scheduler для watch-скриптов чатбота HelpDesk.
Доменная логика (IntraService API, IEK LLM, комментарии, AI-KB) остаётся в <strong>Python</strong>.</p>
<p>Исходник: <code>chatbot_intraservice/docs/интеграции/n8n_migration.md</code> ·
обновление: <code>python scripts/publish_n8n_migration_confluence.py</code>.</p>
<p>Свод проекта:
        ...
        if chunk.startswith("```"):
            ...
            if lang == "mermaid":
                parts.append(
                    ...
                    "</ac:structured-macro>"
                    "<p><em>Схема Mermaid — для draw.io / Confluence diagram; "
                    "исходник в git.</em></p>"
                )
            else:
                parts.append(
        ...
        json.dumps(
            {
                ...
                "space": SPACE,
                "url": link,
                "source": "docs/интеграции/n8n_migration.md",
            },
            ensure_ascii=False,
            indent=2,
        )
